        ...
        <div class="col-md-7">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.property-type.update',$propertyType->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('patch')
                        <div class="form-group">
                            <label for="type">{{ $websiteLang->where('lang_key','type')->first()->custom_text }}</label>
                            <input type="text" name="type" class="form-control" id="type" value="{{ $propertyType->type }}">
                        </div>
                        <div class="form-group">
                            <label for="slug">{{ $websiteLang->where('lang_key','slug')->first()->custom_text }}</label>
                            <input type="text" name="slug" class="form-control" id="slug" value="{{ $propertyType->slug }}">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" id="status" class="form-control">
                                <option {{ $propertyType->status==1 ? 'selected' : '' }} value="1">{{ $websiteLang->where('lang_key','active')->first()->custom_text }} </option>
                                <option {{ $propertyType->status==0 ? 'selected' : '' }}  value="0">{{ $websiteLang->where('lang_key','inactive')->first()->custom_text }}</option>
                            </select>
                        </div>
