...
@section('admin-content')
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('admin.country-state.update',$countryState->id) }}" method="post">
                @csrf
                @method('patch')
                ...
                <div class="form-group">
                    ...
                    <select name="country"  class="form-control" id="custom-select2">
                        ...
                        @foreach ($countries as $item)
                            <option {{ $countryState->country_id==$item->id ? 'selected' : '' }} value="{{ $item->id }}">{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="name" id="name" value="{{ $countryState->name }}">
                </div>
                <div class="form-group">
                    <label for="status">{{ $websiteLang->where('lang_key','status')->first()->custom_text }}</label>
                    <select name="status" id="status" class="form-control">
                        <option {{ $countryState->status==1 ? 'selected': '' }} value="1">{{ $websiteLang->where('lang_key','active')->first()->custom_text }}</option>
                        <option {{ $countryState->status==0 ? 'selected': '' }} value="0">{{ $websiteLang->where('lang_key','inactive')->first()->custom_text }}</option>
                    </select>
                </div>
