                            ...
                            <div class="wsus__sidebar_center">
                                <form method="GET" action="{{ route('search-property') }}">
                                    <div class="wsus__single_input">
                                        ...
                                        <input type="text" placeholder="{{ $websiteLang->where('lang_key','search_type')->first()->custom_text }}" name="search" value="{{ request()->has('search') ? request()->get('search') : '' }}">
                                    </div>
                                    ...
                                    <div class="wp_search_area">
                                        ...
                                        <select class="select_2" name="property_type">
                                            ...
                                            <option {{ request()->get('property_type') == $property_type_item->id ? 'selected' : ''  }} value="{{ $property_type_item->id }}">{{ $property_type_item->type }}</option>
                                            @else
                                    ...
                                    <div class="wp_search_area">
                                        ...
                                        <select class="select_2" name="purpose_type">
                                            ...
                                            <option {{ request()->get('purpose_type') == 1 ? 'selected' : ''  }} value="1">{{ $websiteLang->where('lang_key','sell')->first()->custom_text }}</option>
                                            <option {{ request()->get('purpose_type') == 2 ? 'selected' : ''  }} value="2">{{ $websiteLang->where('lang_key','rent')->first()->custom_text }}</option>
                                            @else
                                            <option value="">{{ $websiteLang->where('lang_key','any')->first()->custom_text }}</option>
                                    ...
                                    <div class="accordion accordion-flush" id="accordionFlushExample">
                                        <div class="accordion-item">
                                            <h2 class="accordion-header" id="flush-headingOne">
                                                <button class="accordion-button {{ $isCollapse ? '' : 'collapsed' }}" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseOne" aria-expanded="false" aria-controls="flush-collapseOne">
                                                    {{ $websiteLang->where('lang_key','aminities')->first()->custom_text }}
                                                </button>
                                            </h2>


                                            <div id="flush-collapseOne" class="accordion-collapse collapse {{ $isCollapse ? 'show' : '' }}" aria-labelledby="flush-headingOne" data-bs-parent="#accordionFlushExample">
                                                    ...
                                                    <div class="wsus__accordian_body">
                                                        @if (request()->has('aminity'))
                                                            ...
                                                            <div class="form-check">
                                                                <input {{ $isChecked ? 'checked' : '' }} name="aminity[]" value="{{ $aminity->id }}" class="form-check-input" type="checkbox" id="aminity_id-{{ $aminity->id }}">
                ...
                <div class="col-xl-8 col-md-8 col-lg-9">
                    <div class="wsus__property_item">
                        ...
                        @php
                            ...
                            foreach ($propertyAminities as $value) {
                                if($value->property->expired_date==null){
                                    $isActivePropertyQty +=1;
                                }else if($value->property->expired_date >= date('Y-m-d')){
                                    $isActivePropertyQty +=1;
                                }
                            }
                        @endphp
                        ...
                        <div class="row">
                            @if ($page_type=='list_view')
                            <div class="col-xl-12">
                                @foreach ($propertyAminities as $item)
                                    @if($item->property->expired_date==null)
                                        <div class="wsus__list_item">
                                            <a href="{{ route('property.details',$item->property->slug) }}" class="wsus__list_item_img">
                                                <img src="{{ $item->property->thumbnail_image }}" alt="property" class="img-fluid w-100">
                                                @if ($item->property->property_purpose_id==1)
                                                <span class="mark">{{ $item->property->propertyPurpose->custom_purpose }}</span>
                                                @elseif($item->property->property_purpose_id==2)
                                                <span class="mark">{{ $item->property->propertyPurpose->custom_purpose }}</span>
                                                @endif

                                                @if ($item->property->urgent_property==1)
                                                    <span class="urgent">{{ $websiteLang->where('lang_key','urgent')->first()->custom_text }}</span>
                                                @endif
                                                {{-- <span class="overlay"></span> --}}
                                            </a>
                                            <div class="wsus__list_item_text">
                                                <p><b>{{ $item->property->propertyType->type }}</b>
                                                        @if ($item->property->property_purpose_id==1)
                                                            <span>{{ $currency->currency_icon }}{{ $item->property->price }}</span>
                                                        @elseif ($item->property->property_purpose_id==2)
                                                        <span>{{ $currency->currency_icon }}{{ $item->property->price }}/
                                                            @if ($item->property->period=='Daily')
                                                            <small class="property-period">{{ $websiteLang->where('lang_key','daily')->first()->custom_text }}</small>
                                                            @elseif ($item->property->period=='Monthly')
                                                            <small class="property-period">{{ $websiteLang->where('lang_key','monthly')->first()->custom_text }}</small>
                                                            @elseif ($item->property->period=='Yearly')
                                                ...
                                                <a href="{{ route('property.details',$item->property->slug) }}"><h4>{{ $item->property->title }}</h4></a>
                                                <ul>
                                                    <li><i class="fal fa-bed"></i> {{ $item->property->number_of_bedroom }} {{ $websiteLang->where('lang_key','bed')->first()->custom_text }}</li>
                                                    <li><i class="fal fa-shower"></i> {{ $item->property->number_of_bathroom }} {{ $websiteLang->where('lang_key','bath')->first()->custom_text }}</li>
                                                    <li><i class="fal fa-draw-square"></i> {{ $item->property->area }} {{ $websiteLang->where('lang_key','sqft_s')->first()->custom_text }}</li>
                                                </ul>
                                                <div class="wsus__rating_n_btn">
                                                    ...
                                                    @if ($total_review > 0)
                                                        ...
                                                        @endif
                                                    <a class="common_btn" href="{{ route('property.details',$item->property->slug) }}">{{ $websiteLang->where('lang_key','view_detail')->first()->custom_text }}</a>
                                                </div>
                                            </div>
                                        </div>
                                    @elseif($item->property->expired_date >= date('Y-m-d'))
                                        <div class="wsus__list_item">
                                            ...
                                            <div class="wsus__list_item_text">
                                                ...
                                                <h4> <a href="{{ route('property.details',$item->property->slug) }}"> {{ $item->property->title }} </a></h4>
                                                <ul>
                                                    <li><i class="fal fa-bed"></i> {{ $item->property->number_of_bedroom }} {{ $websiteLang->where('lang_key','bed')->first()->custom_text }}</li>
                                                    <li><i class="fal fa-shower"></i> {{ $item->property->number_of_bathroom }} {{ $websiteLang->where('lang_key','bath')->first()->custom_text }}</li>
                                                    <li><i class="fal fa-draw-square"></i> {{ $item->property->area }} {{ $websiteLang->where('lang_key','sqft_s')->first()->custom_text }}</li>
                                    ...
                                    @if($item->property->expired_date==null)
                                        <div class="col-xl-6 col-lg-6 ">
                                            <div class="wsus__single_property">
                                                <a class="wsus__single_pro_img" href="{{ route('property.details',$item->property->slug) }}">
                                                    <img src="{{ $item->property->thumbnail_image }}" alt="property" class="img-fluid w-100">
                                                    @if ($item->property->property_purpose_id==1)
                                                    <span class="mark">{{ $item->property->propertyPurpose->custom_purpose }}</span>
                                                    @elseif($item->property->property_purpose_id==2)
                                                ...
                                                <div class="wsus__single_pro_text">
                                                    <p>{{ $item->property->propertyType->type }}
                                                        @php
                                                            $total_review=$item->property->reviews->where('status',1)->count();
                                                            if($total_review > 0){
                                                                $avg_sum=$item->property->reviews->where('status',1)->sum('avarage_rating');
